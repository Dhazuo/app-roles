<template>
    <admin-layout>
        <template v-slot:content>
            <h1 class="text-2xl font-bold mb-16">Editando producto: {{ product.name }}</h1>
            <div class="w-full space-y-4">
                ...
                <div class="space-x-2 flex justify-center">
                    ...
                    <button :disabled="action_state == false" @click="save"
                            :class="{'bg-green-500 focus:bg-green-600 focus:ring-2 focus:ring-green-600 ' : action_state == true, 'bg-gray-200' : action_state == false}"
                            class="rounded-md text-white px-4 py-2 transition duration-200">Editar
                    </button>
                </div>
            </div>
        </template>
